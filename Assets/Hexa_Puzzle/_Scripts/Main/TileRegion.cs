...
public class TileRegion : MonoBehaviour
{
    ...
    private Dictionary<Vector2, Tilesss> slots = new Dictionary<Vector2, Tilesss>();
    ...
    private void LoadBoardBackground()
    {
        for (int col = 0; col < size.y; col++)
        {
            ...
            {
                Tilesss tile = Instantiate(MonoUtils.instance.tile_background);
                tile.transform.SetParent(transform);
                tile.transform.localScale = Vector3.one;
                Vector3 position = col % 2 == 0 ? new Vector3(col * 1.5f * Tilesss.EDGE_SIZE, row * Tilesss.HEIGHT) :
                        new Vector3(col * 1.5f * Tilesss.EDGE_SIZE, row * Tilesss.HEIGHT + Tilesss.HEIGHT / 2);
    ...
    public void Load(GameLevel gameLevel)
    {
        ...
        foreach (var value in positions)
        {
            ...
            Tilesss tile = Instantiate(MonoUtils.instance.tile_background2);
        ...
        foreach (var value in positions)
        {
            ...
            Tilesss tile = Instantiate(values.Length == 2 ? MonoUtils.instance.tile_background : MonoUtils.instance.tile_stone);
            tile.transform.SetParent(MonoUtils.instance.backgroundTilesTransform);
            tile.transform.localScale = Vector3.one;
            Vector3 position = GetLocalPosition(col, row);
            tile.transform.localPosition = position;
        ...
        float regionWidth = maxX - minX + Tilesss.WIDTH + (minX - Tilesss.WIDTH / 2) * 2; ;
        float regionHeight = maxY - minY + Tilesss.HEIGHT + (minY - Tilesss.HEIGHT / 2) * 2;
    ...
    private void LoadPieces(GameLevel gameLevel)
    {
        ...
        foreach (var oneData in data)
        {
            ...
            {
                Tilesss tile = Instantiate(MonoUtils.instance.tile_background_bottom);
    ...
    private Piece CreatePiece(List<Vector2> positions, Transform parent)
    {
        ...
        foreach (var position in piece.defaultPositions)
        {
            ...
            Tilesss tile = Instantiate(MonoUtils.instance.tile_normal);
            ...
            // Add shadows
            Tilesss tileS = Instantiate(MonoUtils.instance.tile_shadow);
    ...
    {
        return col % 2 == 0 ? new Vector3((col * 1.5f + 1) * Tilesss.EDGE_SIZE, (row + 0.5f) * Tilesss.HEIGHT) :
                        new Vector3((col * 1.5f + 1) * Tilesss.EDGE_SIZE, (row + 1) * Tilesss.HEIGHT);
    }
    ...
    public bool CheckMatch(Piece piece)
    {
        ...
        Tilesss matchSlot = null;

        foreach (Tilesss slot in slots.Values)
        {
        ...
        piece.matches = new List<Tilesss>();
    ...
    public void CheckGameComplete()
    {
        ...
        if (isComplete)
        {
            ...
            List<Tilesss> fadedTiles = GetTileOnBoard();
    ...
    private List<Tilesss> GetTileOnBoard()
    {
        List<Tilesss> result = new List<Tilesss>();
    ...
    private IEnumerator FadeTiles(List<Tilesss> fadedTiles)
    {
        foreach (Tilesss tile in fadedTiles)
        {
    ...
    public class TileComparer : IComparer<Tilesss>
    {
        public int Compare(Tilesss x, Tilesss y)
        {
            var fx = F(x);
            var fy = F(y);
        ...
        double F(Tilesss a) // your calculation
        {
